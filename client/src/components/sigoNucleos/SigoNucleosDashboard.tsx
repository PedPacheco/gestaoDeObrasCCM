...
export default function SigoNucleosDashboard() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      localStorage.setItem(
        "sigo_nucleos_data_v2",
        JSON.stringify(INITIAL_NUCLEOS),
      );
    }
  }, []);
  ...
  const handleUpdateField = (id: string, field: keyof Nucleo, value: any) => {
    ...
    triggerNotification(
      `Campo ${field} atualizado inline com sucesso!`,
      "success",
    );
  };
  ...
  useEffect(() => {
    ...
    formValues.chiLimiteBtzero,
    formValues.chiConsumidoBtzero,
    selectedNucleo,
  ]);
    ...
    setNewRow((prev) => {
      ...
      if (field === "chiLimiteBtzero" || field === "chiConsumidoBtzero") {
        next.chiDisponivelBtZero = Math.max(
          0,
          Number(next.chiLimiteBtzero || 0) -
            Number(next.chiConsumidoBtzero || 0),
        );
      }
  ...
  const handleCommitNewRow = () => {
    ...
    if (!nome) {
      ...
      triggerNotification(
        "Informe o nome do núcleo para cadastrar a linha.",
        "error",
      );
      return;
    }
    if (
      nucleos.some((n) => n.nucleo.trim().toLowerCase() === nome.toLowerCase())
    ) {
      setNewRowError("Núcleo já cadastrado.");
      triggerNotification(
        "Este nome de núcleo já está cadastrado no sistema.",
        "error",
      );
      return;
    }
  ...
  const filteredNucleos = useMemo(() => {
    return nucleos
      .filter((item) => {
        ...
        const matchesRegional = filterRegional
          ? item.regional === filterRegional
          : true;
        const matchesStatus = filterStatus
          ? item.statusNucleo === filterStatus
          : true;
        const matchesPrioridade = filterPrioridade
          ? item.prioridadeFinalizacao === filterPrioridade
          : true;
        ...
        if (quickFilter === "restricoes") {
          ...
          matchesQuick =
            item.chiLimiteBtzero > 0 &&
            (remaining <= 0 || remaining / item.chiLimiteBtzero <= 0.15);
  ...
  const handleResetData = () => {
    if (
      window.confirm(
        "Deseja realmente restaurar os dados originais da planilha? Todas as alterações manuais serão perdidas.",
      )
    ) {
      saveToStorage(INITIAL_NUCLEOS);
      setSelectedNucleo(null);
      triggerNotification(
        "Dados de núcleos restaurados para o padrão com sucesso!",
        "info",
      );
    }
  };
  ...
  const handleExportCSV = () => {
    try {
      const headers = [
        ...
        "Resp pela Entrega",
        "Parceira SIGO",
        "Parceira Responsavel",
        "Envio Pendencia Relatorio Final (data)",
        "Prazo Conclusao Pendencias Relatorio Final (data)",
        "Ligacoes Executadas Campo",
        "Ligacoes Notas Baixadas",
        "Meio Ambiente",
        "Quant Restricao Clientes Meio Ambiente",
        "Poder Publico",
        "Quant Restricao Clientes Poder Publico",
        "CHI",
        "Quant Restricao CHI",
        "Conjunto",
        "CHI Necessario",
        "CHI Disponivel BTZero",
        "CHI Limite Btzero",
        "CHI Consumido Btzero",
        "Oportunidade de CHI",
        "Status de andamento Construcao %",
        "Prazo conclusao Construcao (data)",
        "Status de andamento Regularizacao %",
        "Prazo conclusao Regularizacao (data)",
        "Status de andamento Desativação %",
        "Prazo conclusao Desativacao (data)",
        "Prioridade de finalizacao",
        "Observacoes gerais",
      ];
      ...
      filteredNucleos.forEach((item) => {
        const row = [
          ...
          item.tecnologia,
          item.statusNucleo,
          item.dataEntregaPerdas,
          item.respEntrega,
          item.parceiraSigo,
          item.parceiraResponsavel,
          item.envioPendenciaRelatorioFinal,
          item.prazoConclusaoPendenciasRelatorioFinal,
          item.ligacoesExecutadasCampo,
          item.ligacoesNotasBaixadas,
          item.meioAmbienteStatus,
          item.meioAmbienteRestricaoClientes,
          item.poderPublicoStatus,
          ...
          item.prazoConclusaoConstrucao,
          `${item.statusAndamentoRegularizacao}%`,
          item.prazoConclusaoRegularizacao,
          `${item.statusAndamentoDesativacao}%`,
          item.prazoConclusaoDesativacao,
          item.prioridadeFinalizacao,
          item.observacoesGerais.replace(/;/g, ",").replace(/\n/g, " "),
        ];
      ...
      link.setAttribute(
        "download",
        `sigo_nucleos_sms_geral_${new Date().toISOString().slice(0, 10)}.csv`,
      );
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      triggerNotification(
        "Planilha exportada com sucesso como CSV delimitado por ponto e vírgula (;)",
        "success",
      );
    } catch (e) {
  ...
  const handleSaveForm = (e: React.FormEvent) => {
    ...
    if (selectedNucleo) {
      ...
      setSelectedNucleo(null);
      triggerNotification(
        `Núcleo ${formValues.nucleo} atualizado com sucesso!`,
      );
    }
  };
  ...
  const handleDeleteNucleo = (id: string, name: string) => {
    if (
      window.confirm(
        `Tem certeza que deseja excluir permanentemente o núcleo "${name}" do SIGO?`,
      )
    ) {
  ...
  const handleDuplicateNucleo = (item: Nucleo) => {
    ...
    triggerNotification(
      `Núcleo ${item.nucleo} duplicado com sucesso como cópia!`,
      "success",
    );
  };
  ...
  const stats = useMemo(() => {
    ...
    const avgConstruction =
      total > 0
        ? Math.round(
            nucleos.reduce(
              (acc, curr) => acc + curr.statusAndamentoConstrucao,
              0,
            ) / total,
          )
        : 0;
    ...
    const activeRestrictions = nucleos.reduce((acc, curr) => {
      const maRest =
        curr.meioAmbienteStatus === "Pendente"
          ? curr.meioAmbienteRestricaoClientes
          : 0;
      const ppRest =
        curr.poderPublicoStatus === "Pendente"
          ? curr.poderPublicoRestricaoClientes
          : 0;
      const chiRest =
        curr.chiStatus === "Pendente" ? curr.chiRestricaoClientes : 0;
      return acc + maRest + ppRest + chiRest;
    }, 0);

    const totalLimiteChi = nucleos.reduce(
      (acc, curr) => acc + curr.chiLimiteBtzero,
      0,
    );
    const totalConsumidoChi = nucleos.reduce(
      (acc, curr) => acc + curr.chiConsumidoBtzero,
      0,
    );
  ...
  const chartsData = useMemo(() => {
    ...
    REGIONAIS.forEach((r) => {
      regionalProgress[r] = { sum: 0, count: 0 };
    });
    ...
    >
      <div className="max-w-[1600px] w-full mx-auto p-4 flex flex-col gap-5">
        ...
        <header className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl border border-white/5 shadow-xl">
          <div className="px-5 py-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            {/* Tab Selection */}
            <div
              className="flex items-center p-1 rounded-lg border border-white/10 w-fit self-start md:self-auto"
              style={{ background: SURFACE_HEAD }}
            >
              <button
                onClick={() => setActiveTab("tabela")}
                className={tabBtnCls("tabela")}
              >
              ...
              <button
                ...
                className={tabBtnCls("desenvolvedor")}
              >
        ...
        {activeTab === "tabela" && (
          <div className="space-y-4">
            ...
            >
                ...
                <table className="w-full text-left border-collapse text-xs min-w-[3300px]">
                  <thead className="sticky top-0 z-20">
                    ...
                    >
                      {/* Ações */}
                      <th
                        className="w-10 border-r border-white/8 px-2 py-3"
                        style={{ background: SURFACE_HEAD }}
                      >
                      ...
                      <th
                        ...
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[150px] border-r border-white/8"
                      >
                      ...
                      <th
                        ...
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[160px] border-r border-white/8"
                      >
                        <div className="flex items-center gap-1 text-emerald-400">
                      ...
                      <th
                        onClick={() => toggleSort("nucleo")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[190px] border-r border-white/8"
                      >
                        <div className="flex items-center gap-1 text-emerald-400">
                      ...
                      <th
                        onClick={() => toggleSort("tipoRede")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[120px] border-r border-white/8 text-slate-300"
                      >
                        Tipo de rede
                      </th>
                      <th
                        onClick={() => toggleSort("tecnologia")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[120px] border-r border-white/15 text-slate-300"
                      >
                        Tecnologia
                      </th>
                      ...
                      {/* 2. Status & Delivery */}
                      <th
                        onClick={() => toggleSort("statusNucleo")}
                        className="px-3 py-3 bg-amber-500/5 hover:bg-amber-500/10 cursor-pointer w-[170px] border-r border-white/8 text-amber-400"
                      >
                        Status do núcleo
                      </th>
                      <th
                        onClick={() => toggleSort("dataEntregaPerdas")}
                        className="px-3 py-3 bg-amber-500/5 hover:bg-amber-500/10 cursor-pointer w-[150px] border-r border-white/8 text-amber-400"
                      >
                        Data entrega a Perdas
                      </th>
                      <th
                        onClick={() => toggleSort("respEntrega")}
                        className="px-3 py-3 bg-amber-500/5 hover:bg-amber-500/10 cursor-pointer w-[170px] border-r border-white/15 text-amber-400"
                      >
                        Resp. pela entrega
                      </th>
                      ...
                      {/* 3. Partners */}
                      <th
                        onClick={() => toggleSort("parceiraSigo")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[150px] border-r border-white/8 text-slate-300"
                      >
                        Parceira SIGO
                      </th>
                      <th
                        onClick={() => toggleSort("parceiraResponsavel")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[160px] border-r border-white/15 text-slate-300"
                      >
                      ...
                      {/* 5. Connections */}
                      <th
                        onClick={() => toggleSort("ligacoesExecutadasCampo")}
                        className="px-3 py-3 bg-blue-500/5 hover:bg-blue-500/10 cursor-pointer w-[160px] text-right border-r border-white/8 text-blue-400"
                      >
                        Ligações (Exec. campo)
                      </th>
                      <th
                        onClick={() => toggleSort("ligacoesNotasBaixadas")}
                        className="px-3 py-3 bg-blue-500/5 hover:bg-blue-500/10 cursor-pointer w-[160px] text-right border-r border-white/15 text-blue-400"
                      >
                      ...
                      {/* 6. Restrictions */}
                      <th
                        onClick={() => toggleSort("meioAmbienteStatus")}
                        className="px-3 py-3 bg-rose-500/5 hover:bg-rose-500/10 cursor-pointer w-[140px] border-r border-white/8 text-rose-400"
                      >
                        Meio Ambiente
                      </th>
                      <th
                        onClick={() =>
                          toggleSort("meioAmbienteRestricaoClientes")
                        }
                        className="px-3 py-3 bg-rose-500/5 hover:bg-rose-500/10 cursor-pointer w-[150px] text-right border-r border-white/8 text-rose-400"
                      >
                        Quant. Restrição (clientes)
                      </th>
                      <th
                        onClick={() => toggleSort("poderPublicoStatus")}
                        className="px-3 py-3 bg-rose-500/5 hover:bg-rose-500/10 cursor-pointer w-[140px] border-r border-white/8 text-rose-400"
                      >
                        Poder Público
                      </th>
                      <th
                        onClick={() =>
                          toggleSort("poderPublicoRestricaoClientes")
                        }
                        className="px-3 py-3 bg-rose-500/5 hover:bg-rose-500/10 cursor-pointer w-[150px] text-right border-r border-white/8 text-rose-400"
                      >
                        Quant. Restrição (clientes)
                      ...
                      <th
                        onClick={() => toggleSort("chiStatus")}
                        className="px-3 py-3 bg-rose-500/5 hover:bg-rose-500/10 cursor-pointer w-[120px] border-r border-white/8 text-rose-400"
                      >
                        CHI
                      </th>
                      <th
                        onClick={() => toggleSort("chiRestricaoClientes")}
                        className="px-3 py-3 bg-rose-500/5 hover:bg-rose-500/10 cursor-pointer w-[150px] text-right border-r border-white/15 text-rose-400"
                      >
                        Quant. Restrição (CHI)
                      </th>
                      ...
                      {/* 7. Conjunto & CHI */}
                      <th
                        onClick={() => toggleSort("conjunto")}
                        className="px-3 py-3 bg-violet-500/5 hover:bg-violet-500/10 cursor-pointer w-[180px] border-r border-white/8 text-violet-400"
                      >
                        Conjunto
                      </th>
                      <th
                        onClick={() => toggleSort("chiNecessario")}
                        className="px-3 py-3 bg-violet-500/5 hover:bg-violet-500/10 cursor-pointer w-[130px] text-right border-r border-white/8 text-violet-400"
                      >
                        CHI Necessário
                      </th>
                      <th
                        onClick={() => toggleSort("chiDisponivelBtZero")}
                        className="px-3 py-3 bg-violet-500/5 hover:bg-violet-500/10 cursor-pointer w-[150px] text-right border-r border-white/8 text-violet-400"
                      >
                        CHI Disponível BTZero
                      </th>
                      <th
                        onClick={() => toggleSort("chiLimiteBtzero")}
                        className="px-3 py-3 bg-violet-500/5 hover:bg-violet-500/10 cursor-pointer w-[140px] text-right border-r border-white/8 text-violet-400"
                      >
                        CHI Limite Btzero
                      </th>
                      <th
                        onClick={() => toggleSort("chiConsumidoBtzero")}
                        className="px-3 py-3 bg-violet-500/5 hover:bg-violet-500/10 cursor-pointer w-[150px] text-right border-r border-white/8 text-violet-400"
                      >
                        CHI Consumido Btzero
                      </th>
                      <th
                        onClick={() => toggleSort("oportunidadeChi")}
                        className="px-3 py-3 bg-violet-500/5 hover:bg-violet-500/10 cursor-pointer w-[150px] border-r border-white/15 text-violet-400"
                      >
                        Oportunidade de CHI
                      </th>
                      ...
                      <th
                        onClick={() => toggleSort("statusAndamentoConstrucao")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[170px] text-right border-r border-white/8 text-slate-300"
                      >
                        Status Construção %
                      </th>
                      <th
                        onClick={() => toggleSort("prazoConclusaoConstrucao")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[150px] border-r border-white/8 text-slate-300"
                      >
                        Prazo Construção
                      </th>
                      <th
                        onClick={() =>
                          toggleSort("statusAndamentoRegularizacao")
                        }
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[175px] text-right border-r border-white/8 text-slate-300"
                      >
                        Status Regularização %
                      </th>
                      <th
                        onClick={() =>
                          toggleSort("prazoConclusaoRegularizacao")
                        }
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[150px] border-r border-white/8 text-slate-300"
                      >
                        Prazo Regularização
                      ...
                      <th
                        onClick={() => toggleSort("statusAndamentoDesativacao")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[170px] text-right border-r border-white/8 text-slate-300"
                      >
                        Status Desativação %
                      </th>
                      <th
                        onClick={() => toggleSort("prazoConclusaoDesativacao")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[150px] border-r border-white/8 text-slate-300"
                      >
                        Prazo Desativação
                      </th>
                      <th
                        onClick={() => toggleSort("prioridadeFinalizacao")}
                        className="px-3 py-3 hover:bg-white/5 cursor-pointer w-[140px] border-r border-white/8 text-slate-300"
                      >
                        Prioridade finalização
                      </th>
                      <th
                        onClick={() =>
                          toggleSort("envioPendenciaRelatorioFinal")
                        }
                        className="px-3 py-3 bg-blue-500/5 hover:bg-blue-500/10 cursor-pointer w-[180px] border-r border-white/8 text-blue-400/80"
                      >
                        Envio de pendência (data)
                      </th>
                      <th
                        onClick={() =>
                          toggleSort("prazoConclusaoPendenciasRelatorioFinal")
                        }
                        className="px-3 py-3 bg-blue-500/5 hover:bg-blue-500/10 cursor-pointer w-[185px] border-r border-white/15 text-blue-400/80"
                      >
                    ...
                    <tr className="border-b-2 border-white/15 bg-blue-500/[0.06]">
                      ...
                      <td className="p-1 border-r border-white/8">
                        <select
                          ...
                          onChange={(e) =>
                            handleNewRowChange("regional", e.target.value)
                          }
                      ...
                      <td className="p-1 border-r border-white/8">
                        <select
                          ...
                          onChange={(e) =>
                            handleNewRowChange("municipio", e.target.value)
                          }
                        ...
                        >
                          {(MUNICIPIOS[newRow.regional || ""] || []).map(
                            (m) => (
                      ...
                      <td className="p-1 border-r border-white/8">
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter") handleCommitNewRow();
                          }}
                          placeholder="NUC-SPO-..."
                        ...
                        />
                        {newRowError && (
                          <span className="block text-[10px] font-bold text-rose-400 mt-0.5 px-0.5">
                            {newRowError}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="p-1 border-r border-white/8">
                        <select
                          ...
                          onChange={(e) =>
                            handleNewRowChange("tipoRede", e.target.value)
                          }
                          className={newCellInputCls}
                          style={{
                            background: SURFACE_INPUT,
                      ...
                      <td className="p-1 border-r border-white/15">
                        <select
                          ...
                          onChange={(e) =>
                            handleNewRowChange("tecnologia", e.target.value)
                          }
                          className={newCellInputCls}
                          style={{
                            background: SURFACE_INPUT,
                      ...
                      <td className="p-1 border-r border-white/8">
                        <select
                          ...
                          onChange={(e) =>
                            handleNewRowChange("statusNucleo", e.target.value)
                          }
                          className={newCellInputCls}
                          style={{
                            background: SURFACE_INPUT,
                      ...
                      <td className="p-1 border-r border-white/15">
                        <input
                          ...
                          onChange={(e) =>
                            handleNewRowChange("respEntrega", e.target.value)
                          }
                      ...
                      <td className="p-1 border-r border-white/8">
                        <select
                          ...
                          onChange={(e) =>
                            handleNewRowChange("parceiraSigo", e.target.value)
                          }
                          className={newCellInputCls}
                          style={{
                            background: SURFACE_INPUT,
                      ...
                      <td className="p-1 border-r border-white/8">
                        <select
                          ...
                          onChange={(e) =>
                            handleNewRowChange("chiStatus", e.target.value)
                          }
                          className={newCellInputCls}
                          style={{
                            background: SURFACE_INPUT,
                      ...
                      <td className="p-1 border-r border-white/8">
                        <select
                          ...
                          onChange={(e) =>
                            handleNewRowChange("conjunto", e.target.value)
                          }
                          className={newCellInputCls}
                          style={{
                            background: SURFACE_INPUT,
                      ...
                      <td className="p-1 border-r border-white/8">
                        <div
                          className="w-full text-xs font-mono font-bold text-violet-300 py-1.5 px-1.5 rounded-md border border-white/5 text-right"
                      ...
                      <td className="p-1 border-r border-white/15">
                        <input
                          type="date"
                          value={
                            newRow.prazoConclusaoPendenciasRelatorioFinal || ""
                          }
                          onChange={(e) =>
                            handleNewRowChange(
                              "prazoConclusaoPendenciasRelatorioFinal",
                    ...
                    {filteredNucleos.length > 0 ? (
                        ...
                        >
                          ...
                          <td className="w-10 border-r border-white/8 p-0 text-center">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                handleDeleteNucleo(item.id, item.nucleo);
                              }}
                          ...
                          <td
                            className="p-1 border-r border-white/8"
                          ...
                          <td className="px-3 py-2.5 font-mono text-white border-r border-white/8">
                            <div className="flex items-center gap-1.5 justify-between">
                              <span className="underline decoration-dotted decoration-blue-400/60 font-bold">
                                {item.nucleo}
                              </span>
                          ...
                          <td className="px-3 py-2.5 border-r border-white/8 bg-amber-500/[0.03]">
                            <span
                              className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border ${
                                item.statusNucleo === "Ativo" ||
                                item.statusNucleo === "Concluído"
                          ...
                          <td
                            className="px-3 py-2.5 border-r border-white/15 text-slate-300 font-medium truncate max-w-[160px] bg-amber-500/[0.03]"
                            title={item.respEntrega}
                          >
                            {item.respEntrega || "—"}
                          </td>
                          ...
                          <td className="px-3 py-2.5 border-r border-white/8 text-right font-mono font-bold text-slate-200 bg-blue-500/[0.03]">
                            {item.ligacoesExecutadasCampo.toLocaleString(
                              "pt-BR",
                            )}
                          </td>
                          ...
                          <td
                            className={`px-3 py-2.5 border-r border-white/8 text-right font-mono font-bold bg-rose-500/[0.03] ${
                              item.meioAmbienteRestricaoClientes > 0 &&
                              item.meioAmbienteStatus === "Pendente"
                                ? "text-rose-400"
                                : "text-slate-400"
                          ...
                          <td
                            className={`px-3 py-2.5 border-r border-white/15 text-right font-mono font-bold bg-rose-500/[0.03] ${
                              item.chiRestricaoClientes > 0 &&
                              item.chiStatus === "Pendente"
                                ? "text-rose-400"
                                : "text-slate-400"
                          ...
                          {/* Conjunto & CHI */}
                          <td
                            className="p-1 border-r border-white/8 bg-violet-500/[0.03]"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <select
                              value={item.conjunto || ""}
                          ...
                          <td
                            className={`px-3 py-2.5 border-r border-white/8 text-right font-mono font-bold bg-violet-500/[0.03] ${
                              item.chiDisponivelBtZero === 0
                                ? "text-rose-400"
                                : "text-slate-200"
                            }`}
                          >
                          ...
                          <td className="px-3 py-2.5 border-r border-white/8 text-right font-mono font-semibold">
                            <div className="flex items-center justify-end gap-1.5">
                              <span
                                className={
                                  item.statusAndamentoConstrucao === 100
                              ...
                              >
                                <div
                                  ...
                                  style={{
                                    width: `${item.statusAndamentoConstrucao}%`,
                                  }}
                                ></div>
                              </div>
                            </div>
                          ...
                          <td className="px-3 py-2.5 border-r border-white/8 text-right font-mono font-semibold">
                            <div className="flex items-center justify-end gap-1.5">
                              <span
                                className={
                                  item.statusAndamentoRegularizacao === 100
                                    ? "text-emerald-400 font-bold"
                                    : "text-slate-300"
                                }
                              >
                              ...
                              >
                                <div
                                  ...
                                  style={{
                                    width: `${item.statusAndamentoRegularizacao}%`,
                                  }}
                                ></div>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center justify-end gap-1.5">
                              ...
                              >
                                <div
                                  ...
                                  style={{
                                    width: `${item.statusAndamentoDesativacao}%`,
                                  }}
                                ></div>
                              </div>
                            </div>
                          ...
                          <td
                            className="px-3 py-2.5 text-slate-500 truncate max-w-[240px]"
                            title={item.observacoesGerais}
                          >
                      ...
                      <tr style={{ background: SURFACE_CARD }}>
                        ...
                        >
                          ...
                          Nenhum núcleo cadastrado. Preencha a linha destacada
                          acima para adicionar o primeiro.
                        </td>
                      </tr>
                    )}
                  </tbody>
              ...
              <div
                className="border-t border-white/8 px-4 py-3 text-xs text-slate-500 flex flex-col sm:flex-row sm:items-center justify-between gap-2"
                style={{ background: SURFACE_HEAD }}
              >
                <div>
                  Exibindo{" "}
                  <span className="font-bold text-slate-200">
                    {filteredNucleos.length}
                  </span>{" "}
                  de{" "}
                  <span className="font-bold text-slate-200">
                    {nucleos.length}
                ...
                <div className="flex items-center gap-4">
                  <span className="flex items-center gap-1">
                    ...
                    Ativos/Concluídos (
                    {
                      nucleos.filter(
                        (n) =>
                          n.statusNucleo === "Ativo" ||
                          n.statusNucleo === "Concluído",
                      ).length
                    }
                    )
                  </span>
                  <span className="flex items-center gap-1">
                    ...
                    Em Construção (
                    {
                      nucleos.filter((n) => n.statusNucleo === "Em Construção")
                        .length
                    }
                    )
                  </span>
                  <span className="flex items-center gap-1">
                    ...
                    {
                      nucleos.filter(
                        (n) => n.statusNucleo === "Pendente Regularização",
                      ).length
                    }
                    )
                  </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-6 border border-white/5 shadow-xl space-y-4">
              ...
              <p className="text-xs text-slate-500">
                Média do percentual de conclusão da fase de construção (%) em
                cada regional cadastrada.
              </p>
            ...
            <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-6 border border-white/5 shadow-xl space-y-4">
              ...
              <p className="text-xs text-slate-500">
                Detalhamento numérico da situação operacional de todos os
                núcleos cadastrados.
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 pt-2">
                {chartsData.statusPie.map((item) => {
                  ...
                  return (
                    <div
                      key={item.label}
                      className="border border-white/8 p-3 rounded-xl text-center flex flex-col justify-center items-center gap-1.5"
                      style={{ background: SURFACE_HEAD }}
                    >
                      <span
                        className="text-[11px] font-bold text-slate-500 uppercase tracking-wide truncate max-w-full"
                        title={item.label}
                      >
                        {item.label}
                      </span>
            ...
            <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-6 border border-white/5 shadow-xl space-y-4 lg:col-span-2">
              ...
              <p className="text-xs text-slate-500">
                Acompanhamento individualizado de saturação de CHI. Núcleos em
                vermelho esgotaram o CHI e necessitam liberação ou possuem
                status crítico.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-2">
                {nucleos.map((n) => {
                  const saturation =
                    n.chiLimiteBtzero > 0
                      ? Math.round(
                          (n.chiConsumidoBtzero / n.chiLimiteBtzero) * 100,
                        )
                      : 0;
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <span
                          className="font-mono text-xs font-bold text-slate-200 truncate max-w-[120px]"
                          title={n.nucleo}
                        >
                          {n.nucleo}
                        </span>
                        ...
                        <div>
                          ...
                          <span
                            className={`block font-mono text-xs font-bold ${
                              n.chiDisponivelBtZero === 0
                                ? "text-rose-400"
                                : "text-slate-200"
                            }`}
                          >
                            {n.chiDisponivelBtZero}
                          </span>
                        </div>
                      </div>
                    </div>
            ...
            <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-5 border border-white/5 shadow-xl flex items-start gap-4">
              ...
              <div className="space-y-1">
                <h2 className="text-base font-bold text-blue-400">
                  Portal do Desenvolvedor de Sistemas SIGO
                </h2>
                <p className="text-xs text-slate-400">
                  O objetivo desta tela é fornecer ao desenvolvedor do SIGO toda
                  a estrutura pronta de banco de dados e APIs necessários para
                  subir esta rotina de acompanhamento dos Núcleos no sistema
                  real.
                </p>
                <div className="flex flex-wrap items-center gap-3 pt-2">
                  <span
                    className="text-[10px] text-slate-400 font-bold px-2 py-0.5 rounded-full border border-white/10"
                    style={{ background: SURFACE_HEAD }}
                  >
            ...
            {[
              {
                ...
                title: "1. Script SQL DDL de Criação da Tabela (PostgreSQL)",
                code: SQL_SCHEMA_POSTGRES,
                label: "Copiar SQL",
                maxH: "max-h-[350px]",
              },
              {
                ...
                title: "2. Script SQL de Seeds (Insert de Exemplo)",
                code: SQL_INSERT_SEEDS,
                label: "Copiar SQL Seeds",
                maxH: "max-h-[300px]",
              },
              {
                key: "express_controller",

                title:
                  "3. Backend Controller & Endpoints (Node.js Express / pg)",
                code: BACKEND_CONTROLLER_CODE,
                label: "Copiar Código",
                maxH: "max-h-[350px]",
              },
            ].map((section) => {
              return (
                ...
                >
                  ...
                  <pre
                    className={`p-4 text-slate-300 text-[11.5px] font-mono overflow-x-auto leading-relaxed ${section.maxH}`}
      ...
      {selectedNucleo && (
        <div className="fixed inset-0 z-40 flex justify-end">
          ...
          <div
            className="relative w-full max-w-2xl h-full shadow-2xl flex flex-col z-10 overflow-hidden border-l border-white/10"
            style={{ background: SURFACE_CARD }}
          >
            {/* Drawer Header */}
            <div
            ...
            >
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      style={{ background: SURFACE_INPUT, colorScheme: "dark" }}
                    >
                      {(MUNICIPIOS[formValues.regional || ""] || []).map(
                        (city) => (
                  ...
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          nucleo: e.target.value.toUpperCase(),
                        }))
                      }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          prioridadeFinalizacao: e.target.value as any,
                        }))
                      }
                      className="w-full text-xs py-2 px-2.5 text-slate-200 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                      style={{ background: SURFACE_INPUT, colorScheme: "dark" }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          dataEntregaPerdas: e.target.value,
                        }))
                      }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          parceiraResponsavel: e.target.value,
                        }))
                      }
                      className="w-full text-xs py-2 px-2 text-slate-200 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                      style={{ background: SURFACE_INPUT, colorScheme: "dark" }}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">
                      Data Envio Pendência Relatório
                    </label>
                    <input
                      type="date"
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          envioPendenciaRelatorioFinal: e.target.value,
                        }))
                      }
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">
                      Prazo de Conclusão Relatório
                    </label>
                    <input
                      type="date"
                      value={
                        formValues.prazoConclusaoPendenciasRelatorioFinal || ""
                      }
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          ligacoesExecutadasCampo: Number(e.target.value),
                        }))
                      }
                      className="w-full text-xs py-2 px-2.5 text-slate-200 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                      style={{ background: SURFACE_INPUT, colorScheme: "dark" }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          ligacoesNotasBaixadas: Number(e.target.value),
                        }))
                      }
                      className="w-full text-xs py-2 px-2.5 text-slate-200 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                      style={{ background: SURFACE_INPUT, colorScheme: "dark" }}
              ...
              <div className="space-y-4 p-4 rounded-xl border border-rose-500/20 bg-rose-500/[0.04]">
                ...
                {(
                  [
                    {
                      statusField: "meioAmbienteStatus",
                      qtyField: "meioAmbienteRestricaoClientes",
                      label: "Meio Ambiente (Status)",
                      qtyLabel: "Quant. Restrição (clientes)",
                    },
                    {
                      statusField: "poderPublicoStatus",
                      qtyField: "poderPublicoRestricaoClientes",
                      label: "Poder Público (Status)",
                      qtyLabel: "Quant. Restrição (clientes)",
                    },
                    {
                      statusField: "chiStatus",
                      qtyField: "chiRestricaoClientes",
                      label: "CHI",
                      qtyLabel: "Quant. Restrição (CHI)",
                    },
                  ] as const
                ).map((row) => (
                  <div
                  ...
                  >
                    <div>
                      ...
                      <select
                        value={
                          (formValues[row.statusField] as string) || "Liberado"
                        }
                        onChange={(e) =>
                          setFormValues((prev) => ({
                            ...prev,
                            [row.statusField]: e.target.value,
                          }))
                        }
                        className="w-full text-xs py-2 px-2 text-slate-200 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                        style={{
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormValues((prev) => ({
                            ...prev,
                            [row.qtyField]: Number(e.target.value),
                          }))
                        }
              ...
              <div className="space-y-4 p-4 rounded-xl border border-violet-500/20 bg-violet-500/[0.04]">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          chiNecessario: Number(e.target.value),
                        }))
                      }
                      className="w-full text-xs py-2 px-2 text-slate-200 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                      style={{ background: SURFACE_INPUT, colorScheme: "dark" }}
                ...
                <div
                  className="grid grid-cols-3 gap-4 p-3 rounded-lg border border-white/8"
                  style={{ background: SURFACE_HEAD }}
                >
                  <div>
                    <label className="block text-xs font-bold text-slate-300 mb-1">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          chiLimiteBtzero: Number(e.target.value),
                        }))
                      }
                      className="w-full text-xs py-2 px-2 text-slate-200 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                      style={{ background: SURFACE_INPUT, colorScheme: "dark" }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setFormValues((prev) => ({
                          ...prev,
                          chiConsumidoBtzero: Number(e.target.value),
                        }))
                      }
                      className="w-full text-xs py-2 px-2 text-slate-200 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                      style={{ background: SURFACE_INPUT, colorScheme: "dark" }}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormValues((prev) => ({
                        ...prev,
                        oportunidadeChi: e.target.value,
                      }))
                    }
                    className="w-full text-xs py-2 px-2.5 text-slate-200 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/40"
                    style={{ background: SURFACE_INPUT, colorScheme: "dark" }}
              ...
              <div className="space-y-4">
                ...
                {(
                  [
                    {
                      pctField: "statusAndamentoConstrucao",
                      dateField: "prazoConclusaoConstrucao",
                      label: "Construção",
                      accent: "accent-emerald-500",
                    },
                    {
                      pctField: "statusAndamentoRegularizacao",
                      dateField: "prazoConclusaoRegularizacao",
                      label: "Regularização",
                      accent: "accent-blue-500",
                    },
                    {
                      pctField: "statusAndamentoDesativacao",
                      dateField: "prazoConclusaoDesativacao",
                      label: "Desativação",
                  ...
                  <div
                    key={row.pctField}
                    className="grid grid-cols-2 gap-4 p-3 rounded-lg border border-white/8"
                    style={{ background: SURFACE_HEAD }}
                  >
                    <div>
                      <label className="block text-xs font-bold text-slate-300 mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setFormValues((prev) => ({
                              ...prev,
                              [row.pctField]: Number(e.target.value),
                            }))
                          }
                          className={`w-full h-1.5 rounded-lg appearance-none cursor-pointer bg-white/10 ${row.accent}`}
                        />
                        <span className="font-mono font-bold text-xs w-10 text-right text-slate-200">
                          {(formValues[row.pctField] as number) || 0}%
                        </span>
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs font-bold text-slate-300 mb-1">
                        Prazo Conclusão {row.label}
                      </label>
                      <input
                        type="date"
            ...
            <div
              className="border-t border-white/8 px-5 py-3.5 flex items-center justify-between"
              style={{ background: SURFACE_HEAD }}
            >
              <button
                type="button"
                onClick={() =>
                  handleDeleteNucleo(selectedNucleo.id, selectedNucleo.nucleo)
                }
