...
function KpiCard({
  ...
  return (
    ...
    >
      ...
      <span className="text-2xl font-black text-white/90 pl-2 leading-tight">
        {value}
      </span>
    ...
    <div className="flex flex-col gap-5 p-6 min-h-full">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <KpiCard
          ...
          onClick={() =>
            setActiveKpi(
              activeKpi === "Total Forecast" ? null : "Total Forecast",
            )
          }
        />
        <KpiCard
          ...
          onClick={() =>
            setActiveKpi(
              activeKpi === "Total Executado" ? null : "Total Executado",
            )
          }
        />
        <KpiCard
          ...
          sub={
            taxaExec >= 100
              ? "Meta atingida"
              : `Faltam ${BRL(totals1.totalForecast - totals1.totalExec)}`
          }
        ...
        <KpiCard
          ...
          value={BRL(
            totals1.totalServiceMoForecast + totals1.totalMaterialMoForecast,
          )}
            ...
            <div>
              ...
              <div className="flex flex-col gap-2.5">
                {[...summary2]
                  ...
                  .map((r, i) => {
                    const maxV = Math.max(
                      ...summary2.map((x) => x.forecastTotal),
                      1,
                    );
                    return (
                      <div key={i} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-[#53FF75] rounded-full"
                            style={{
                              width: `${(r.forecastTotal / maxV) * 100}%`,
                            }}
                          />
                        </div>
                      ...
                      <div key={i} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{ width: `${(r.execTotal / maxV) * 100}%` }}
                          />
                        </div>
                        <span className="text-white font-bold text-xs whitespace-nowrap">
                          {BRL(r.execTotal)}
          ...
          {activeKpi === "Taxa Exec / Forecast" && (
            <div className="grid grid-cols-3 gap-4">
              {[
                {
                  label: "Acima de 100%",
                  count: summary2.filter(
                    (r) =>
                      r.forecastTotal > 0 &&
                      (r.execTotal / r.forecastTotal) * 100 >= 100,
                  ).length,
                  color: "#53FF75",
                },
                {
                  ...
                  count: summary2.filter(
                    (r) =>
                      r.forecastTotal > 0 &&
                      (r.execTotal / r.forecastTotal) * 100 >= 80 &&
                      (r.execTotal / r.forecastTotal) * 100 < 100,
                  ).length,
                  color: "#f59e0b",
                },
          ...
          {activeKpi === "Composição Forecast" && (
            <div className="grid grid-cols-2 gap-6">
              {[
                {
                  label: "Serviço",
                  forecast: totals1.totalServiceMoForecast,
                  exec: totals1.totalServiceMoExec,
                  color: "#53FF75",
                },
                {
                  label: "Material",
                  forecast: totals1.totalMaterialMoForecast,
                  exec: totals1.totalMaterialMoExec,
                  color: "#a78bfa",
                },
      ...
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-1">
          ...
          <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-5 border border-white/5 shadow-xl">
            <h3 className="text-white font-bold text-sm mb-4 uppercase tracking-wide">
              Serviço vs Material — Forecast (dia)
            </h3>
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart
                data={areaByDay}
          ...
          <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-5 border border-white/5 shadow-xl">
            <h3 className="text-white font-bold text-sm mb-4 uppercase tracking-wide">
              Distribuição Forecast por Grupo
            </h3>
                      ...
                      return (
                        <div className="bg-[#0f1e2e]/95 border border-white/10 rounded-xl px-3 py-2 text-xs shadow-2xl">
                          ...
                          <div className="text-zinc-400">
                            ...
                            <span className="text-[#53FF75]">({pct}%)</span>
                          </div>
                        </div>
                      );
                    }}
                ...
                {pieByGrupo.map((g, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-zinc-300 text-xs font-semibold flex-1 truncate">
                        {g.name}
                      </span>
      ...
      <div
        className="overflow-hidden transition-all duration-500 ease-in-out"
        style={{
          maxHeight: openTables ? "2000px" : "0px",
          opacity: openTables ? 1 : 0,
        }}
      >
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-white/5">
                    <th
                      rowSpan={2}
                      className="py-2 px-3 text-left text-zinc-400 font-semibold uppercase tracking-wider whitespace-nowrap"
                    >
                      Data
                    </th>
                    <th
                      rowSpan={2}
                      className="py-2 px-3 text-zinc-400 font-semibold uppercase tracking-wider"
                    >
                      Dia
                    </th>
                    <th
                      colSpan={3}
                      className="py-2 px-3 text-center text-[#53FF75] font-bold uppercase tracking-wider border-x border-[#53FF75]/20 bg-[#53FF75]/5"
                    >
                      Forecast (R$)
                    </th>
                    <th
                      colSpan={3}
                      className="py-2 px-3 text-center text-[#3b82f6] font-bold uppercase tracking-wider border-x border-[#3b82f6]/20 bg-[#3b82f6]/5"
                    >
                      Executado (R$)
                    </th>
                  </tr>
                  <tr className="border-b border-white/5 bg-white/[0.02]">
                    <th className="py-2 px-3 text-[#53FF75]/70 font-semibold text-right border-l border-[#53FF75]/20">
                      Serviço
                    </th>
                    <th className="py-2 px-3 text-[#53FF75]/70 font-semibold text-right">
                      Material
                    </th>
                    <th className="py-2 px-3 text-[#53FF75]/70 font-semibold text-right border-r border-[#53FF75]/20">
                      Total
                    </th>
                    <th className="py-2 px-3 text-[#3b82f6]/70 font-semibold text-right border-l border-[#3b82f6]/20">
                      Serviço
                    </th>
                    <th className="py-2 px-3 text-[#3b82f6]/70 font-semibold text-right">
                      Material
                    </th>
                    <th className="py-2 px-3 text-[#3b82f6]/70 font-semibold text-right border-r border-[#3b82f6]/20">
                      Total
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {summary1.length === 0 ? (
                    ...
                    summary1.map((row, i) => (
                      <tr
                        key={i}
                        className="border-b border-white/[0.04] hover:bg-white/5 transition-colors"
                      >
                        <td className="py-2 px-3 text-[#53FF75] font-semibold whitespace-nowrap">
                          {row.dataProg.substring(0, 5)}
                        </td>
                        <td className="py-2 px-3 text-zinc-400 text-center">
                          {getDayOfWeek(row.dataProg)}
                        </td>
                        <td className="py-2 px-3 text-zinc-300 text-right border-l border-[#53FF75]/10 whitespace-nowrap">
                          {BRL(row.serviceMoForecast)}
                        </td>
                        <td className="py-2 px-3 text-zinc-300 text-right whitespace-nowrap">
                          {BRL(row.materialMoForecast)}
                        </td>
                        <td className="py-2 px-3 text-white font-semibold text-right border-r border-[#53FF75]/10 whitespace-nowrap">
                          {BRL(row.forecastTotal)}
                        </td>
                        <td className="py-2 px-3 text-zinc-300 text-right border-l border-[#3b82f6]/10 whitespace-nowrap">
                          {BRL(row.serviceMoExec)}
                        </td>
                        <td className="py-2 px-3 text-zinc-300 text-right whitespace-nowrap">
                          {BRL(row.materialMoExec)}
                        </td>
                        <td className="py-2 px-3 text-white font-semibold text-right border-r border-[#3b82f6]/10 whitespace-nowrap">
                          {BRL(row.execTotal)}
                        </td>
                      </tr>
                    ))
                ...
                <tfoot>
                  <tr className="border-t-2 border-[#53FF75]/30 bg-[#53FF75]/10">
                    <td
                      colSpan={2}
                      className="py-3 px-3 text-[#53FF75] font-bold uppercase tracking-wide"
                    >
                      Total
                    </td>
                    <td className="py-3 px-3 text-[#53FF75] font-bold text-right whitespace-nowrap">
                      {BRL(totals1.totalServiceMoForecast)}
                    </td>
                    <td className="py-3 px-3 text-[#53FF75] font-bold text-right whitespace-nowrap">
                      {BRL(totals1.totalMaterialMoForecast)}
                    </td>
                    <td className="py-3 px-3 text-[#53FF75] font-bold text-right whitespace-nowrap">
                      {BRL(totals1.totalForecast)}
                    </td>
                    <td className="py-3 px-3 text-[#3b82f6] font-bold text-right whitespace-nowrap">
                      {BRL(totals1.totalServiceMoExec)}
                    </td>
                    <td className="py-3 px-3 text-[#3b82f6] font-bold text-right whitespace-nowrap">
                      {BRL(totals1.totalMaterialMoExec)}
                    </td>
                    <td className="py-3 px-3 text-[#3b82f6] font-bold text-right whitespace-nowrap">
                      {BRL(totals1.totalExec)}
                    </td>
                  </tr>
                </tfoot>
              </table>
              ...
              <table className="w-full text-xs">
                  ...
                  {summary2.length === 0 ? (
                    ...
                    summary2.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="py-2 px-3 text-zinc-300 text-right border-l border-[#53FF75]/10 whitespace-nowrap">
                          {BRL(row.totalServiceMoForecast)}
                        </td>
                        <td className="py-2 px-3 text-zinc-300 text-right whitespace-nowrap">
                          {BRL(row.totalMaterialMoForecast)}
                        </td>
                        <td className="py-2 px-3 text-white font-semibold text-right border-r border-[#53FF75]/10 whitespace-nowrap">
                          {BRL(row.forecastTotal)}
                        </td>
                        <td className="py-2 px-3 text-zinc-300 text-right border-l border-[#3b82f6]/10 whitespace-nowrap">
                          {BRL(row.totalServiceMoExec)}
                        </td>
                        <td className="py-2 px-3 text-zinc-300 text-right whitespace-nowrap">
                          {BRL(row.totalMaterialMoExec)}
                        </td>
                        <td className="py-2 px-3 text-white font-semibold text-right border-r border-[#3b82f6]/10 whitespace-nowrap">
                          {BRL(row.execTotal)}
                        </td>
                ...
                <tfoot>
                  <tr className="border-t-2 border-[#53FF75]/30 bg-[#53FF75]/10">
                    ...
                    <td className="py-3 px-3 text-[#53FF75] font-bold text-right whitespace-nowrap">
                      {BRL(totals2.totalServiceMoForecastByGrouping)}
                    </td>
                    <td className="py-3 px-3 text-[#53FF75] font-bold text-right whitespace-nowrap">
                      {BRL(totals2.totalMaterialMoForecastByGrouping)}
                    </td>
                    <td className="py-3 px-3 text-[#53FF75] font-bold text-right whitespace-nowrap">
                      {BRL(totals2.totalForecast)}
                    </td>
                    <td className="py-3 px-3 text-[#3b82f6] font-bold text-right whitespace-nowrap">
                      {BRL(totals2.totalServiceMoExecByGrouping)}
                    </td>
                    <td className="py-3 px-3 text-[#3b82f6] font-bold text-right whitespace-nowrap">
                      {BRL(totals2.totalMaterialMoExecByGrouping)}
                    </td>
                    <td className="py-3 px-3 text-[#3b82f6] font-bold text-right whitespace-nowrap">
                      {BRL(totals2.totalExec)}
                    </td>
                  </tr>
                </tfoot>
              </table>
