  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        {badge && (
          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#3b82f6]/20 text-[#3b82f6]">{badge}</span>
        )}
      </div>
      <svg
    ...
    <div className="flex items-center gap-4 bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-5 border border-white/5 shadow-xl hover:border-white/10 transition-all">
      <div className="relative shrink-0" style={{ width: 88, height: 88 }}>
        <svg width={88} height={88} viewBox="0 0 88 88">
          <circle cx={44} cy={44} r={radius} fill="none" stroke="#ffffff08" strokeWidth={stroke} />
          <circle
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className="text-xl font-black leading-none" style={{ color }}>{pct.toFixed(0)}%</span>
        </div>
      </div>
      <div className="flex flex-col gap-1 min-w-0">
        ...
        <div className="w-full h-1 bg-white/5 rounded-full mt-1">
          <div className="h-full rounded-full transition-all duration-700" style={{ width: `${visualPct}%`, background: color }} />
        </div>
      </div>
    </div>
  );
...
export default function MaodeObraDashboard({ initialData, initialData2, token, initialMetaDiaria }: Props) {
  ...
  const [filterPerf,    setFilterPerf]      = useState<"all" | "above" | "below">("all");
  ...
  const todayTeams   = data.length > 0 ? (data[data.length - 1]?.teamsTotal ?? 0) : 0;
  ...
  const turmaOptions  = [...new Set(
    data2.filter(r => !selectedGrupo || r.grupo === selectedGrupo).map(r => r.turma)
  )].sort();
  const filteredData2 = data2.filter(r =>
  ...
  const displayProg = isFiltered ? filteredData2.reduce((s, r) => s + r.totalMoProg, 0) : totalProg;
  const displayExec = isFiltered ? filteredData2.reduce((s, r) => s + r.totalMoExec, 0) : totalExec;
  const displayPrev = isFiltered ? filteredData2.reduce((s, r) => s + r.totalMoPrev, 0) : totalMeta100;
  ...
  const pctMeta108  = displayPrev > 0 ? (displayProg / (displayPrev * 1.08)) * 100 : 0;
  ...
  const topParceiraData = [...filteredData2]
    ...
    .map(r => ({
      ...
      Programado: Math.round(r.totalMoProg),
      Executado:  Math.round(r.totalMoExec),
      pct: r.totalMoProg > 0 ? +((r.totalMoExec / r.totalMoProg) * 100).toFixed(1) : 0,
    }));
    ...
    <div className="flex flex-col gap-5 p-6 min-h-full bg-[#111c27]/80 backdrop-blur-sm">
      ...
      <div className="flex flex-wrap items-center gap-4 bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-4 border border-white/5 shadow-xl">
        ...
        {grupoOptions.length > 0 && (
          <div className="flex items-center gap-2 border-l border-white/10 pl-4">
            <div className="relative">
              <select
                value={selectedGrupo}
                onChange={e => { setSelectedGrupo(e.target.value); setSelectedTurma(""); }}
              ...
              <svg className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </div>
            <div className="relative">
            ...
            {(selectedGrupo || selectedTurma) && (
              <button onClick={() => { setSelectedGrupo(""); setSelectedTurma(""); }} className="text-zinc-500 hover:text-zinc-300 text-xs transition-colors">
                Limpar
              </button>
            )}
          </div>
        ...
        <div className="ml-auto flex items-center gap-2 text-zinc-500 text-xs">
          <span className="w-2 h-2 rounded-full bg-[#53FF75] inline-block" /> Semana
          <span className="w-2 h-2 rounded-full bg-[#354a60] inline-block ml-2" /> Fim de semana
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-5 gap-4">
        {[
          { label: "Total Obras",   value: isFiltered ? filteredData2.reduce((s, r) => s + (r.qtdeWorks ?? 0), 0).toLocaleString("pt-BR") : totalObras.toLocaleString("pt-BR"),   sub: "programadas no mês",  grad: "from-[#182638] to-[#1c2f42]", accent: "#60a5fa" },
          { label: "Total Equipes", value: isFiltered ? todayTeams.toLocaleString("pt-BR") : totalEquipes.toLocaleString("pt-BR"), sub: isFiltered ? "hoje na rua" : "mobilizadas",  grad: "from-[#182638] to-[#1c2f42]", accent: "#38bdf8" },
          { label: "Programado",    value: BRL(displayProg),                     sub: "mão de obra",  grad: "from-[#182638] to-[#1c2f42]", accent: "#818cf8" },
          { label: "Executado",     value: BRL(displayExec),                     sub: "mão de obra",  grad: "from-[#182638] to-[#1c2f42]", accent: "#4ade80" },
          { label: "Taxa Execução", value: `${displayTaxa.toFixed(1)}%`,         sub: "exec / prog",  grad: "from-[#182638] to-[#1c2f42]", accent: "#94a3b8" },
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between pl-2">
                <span className="text-white/60 text-sm uppercase tracking-widest font-medium">{label}</span>
        ...
        <div className="bg-gradient-to-br from-[#1a2d42] to-[#182333] rounded-2xl p-5 border border-white/8 shadow-xl">
          ...
          {activeKpi === "Total Obras" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <p className="text-zinc-500 text-xs mb-3 uppercase tracking-wider">Top 5 dias por qtde de obras</p>
                <div className="flex flex-col gap-2.5">
                  {[...data].sort((a, b) => b.totalQtde - a.totalQtde).slice(0, 5).map((d, i) => {
                    const maxQ = Math.max(...data.map(r => r.totalQtde), 1);
                    return (
                      <div key={i} className="flex items-center gap-3">
                        <span className="text-[#4ade80] font-bold text-xs w-14 shrink-0">{d.dataProg.substring(0, 5)}</span>
                        <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                          <div className="h-full bg-[#60a5fa] rounded-full" style={{ width: `${(d.totalQtde / maxQ) * 100}%` }} />
                        </div>
                        <span className="text-white font-bold text-xs w-6 text-right">{d.totalQtde}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="bg-white/5 rounded-xl p-3">
                  <p className="text-zinc-500 text-xs uppercase tracking-wider mb-1">Média diária</p>
                  <p className="text-white font-black text-2xl">{data.length > 0 ? (totalObras / data.length).toFixed(1) : "—"}</p>
                  <p className="text-zinc-500 text-xs">obras por dia</p>
                </div>
                <div className="bg-white/5 rounded-xl p-3">
                  <p className="text-zinc-500 text-xs uppercase tracking-wider mb-1">Máximo em um dia</p>
                  <p className="text-white font-black text-2xl">{data.length > 0 ? Math.max(...data.map(r => r.totalQtde)) : "—"}</p>
          ...
          {activeKpi === "Total Equipes" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <p className="text-zinc-500 text-xs mb-3 uppercase tracking-wider">Top 5 dias por equipes mobilizadas</p>
                <div className="flex flex-col gap-2.5">
                  {[...data].sort((a, b) => (b.teamsTotal ?? 0) - (a.teamsTotal ?? 0)).slice(0, 5).map((d, i) => {
                    const maxT = Math.max(...data.map(r => r.teamsTotal ?? 0), 1);
                    return (
                      <div key={i} className="flex items-center gap-3">
                        <span className="text-[#4ade80] font-bold text-xs w-14 shrink-0">{d.dataProg.substring(0, 5)}</span>
                        <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                          <div className="h-full bg-[#38bdf8] rounded-full" style={{ width: `${((d.teamsTotal ?? 0) / maxT) * 100}%` }} />
                        </div>
                        <span className="text-white font-bold text-xs w-6 text-right">{d.teamsTotal ?? 0}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="bg-white/5 rounded-xl p-3 self-start">
                <p className="text-zinc-500 text-xs uppercase tracking-wider mb-1">Média (dias ativos)</p>
                <p className="text-white font-black text-2xl">
                  {data.filter(r => (r.teamsTotal ?? 0) > 0).length > 0
                    ? (totalEquipes / data.filter(r => (r.teamsTotal ?? 0) > 0).length).toFixed(1)
                    : "—"}
                </p>
          ...
          {activeKpi === "Programado" && (
            <div>
              <p className="text-zinc-500 text-xs mb-3 uppercase tracking-wider">Top 5 parceiras — valor programado</p>
              <div className="flex flex-col gap-2.5">
                {[...filteredData2].sort((a, b) => b.totalMoProg - a.totalMoProg).slice(0, 5).map((r, i) => {
                  const maxV = Math.max(...filteredData2.map(x => x.totalMoProg), 1);
                  return (
                    <div key={i} className="flex items-center gap-3">
                      <span className="text-zinc-300 text-xs w-40 truncate shrink-0">{r.turma}</span>
                      <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <div className="h-full bg-[#818cf8] rounded-full" style={{ width: `${(r.totalMoProg / maxV) * 100}%` }} />
                      </div>
                      <span className="text-white font-bold text-xs whitespace-nowrap">{BRL(r.totalMoProg)}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {activeKpi === "Executado" && (
            <div>
              <p className="text-zinc-500 text-xs mb-3 uppercase tracking-wider">Top 5 parceiras — valor executado</p>
              <div className="flex flex-col gap-2.5">
                {[...filteredData2].sort((a, b) => b.totalMoExec - a.totalMoExec).slice(0, 5).map((r, i) => {
                  const maxV = Math.max(...filteredData2.map(x => x.totalMoExec), 1);
                  return (
                    <div key={i} className="flex items-center gap-3">
                      <span className="text-zinc-300 text-xs w-40 truncate shrink-0">{r.turma}</span>
                      <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                        <div className="h-full bg-[#4ade80] rounded-full" style={{ width: `${(r.totalMoExec / maxV) * 100}%` }} />
                      </div>
                      <span className="text-white font-bold text-xs whitespace-nowrap">{BRL(r.totalMoExec)}</span>
                    </div>
                  );
                })}
              </div>
          ...
          {activeKpi === "Taxa Execução" && (
            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Acima da meta",  count: data.filter(r => metaDiaria > 0 && (r.totalMoProg / metaDiaria) * 100 >= 100).length, color: "#4ade80" },
                { label: "Entre 70–99%",   count: data.filter(r => metaDiaria > 0 && (r.totalMoProg / metaDiaria) * 100 >= 70 && (r.totalMoProg / metaDiaria) * 100 < 100).length, color: "#f97316" },
                { label: "Abaixo de 70%",  count: data.filter(r => metaDiaria > 0 && (r.totalMoProg / metaDiaria) * 100 < 70).length, color: "#818cf8" },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="flex items-center gap-4 bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-5 border border-white/5 shadow-xl">
          <ResponsiveContainer width={88} height={88}>
            <PieChart>
              <Pie data={pieByGroup} dataKey="value" cx="50%" cy="50%" outerRadius={40} innerRadius={22} strokeWidth={0} paddingAngle={2}>
                {pieByGroup.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
              </Pie>
              <Tooltip
      ...
      <div
        ...
        style={{ maxHeight: openCharts ? "700px" : "0px", opacity: openCharts ? 1 : 0 }}
      >
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 pt-1">
          ...
          <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-5 border border-white/5 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-white font-bold text-sm uppercase tracking-wide">Programado vs Executado (dia)</h3>
              {isFiltered && (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <ComposedChart data={barByDay} margin={{ left: 0, right: 10, top: 4, bottom: 0 }} barGap={2}>
                ...
                <XAxis dataKey="dia" tick={{ fill: "#64748b", fontSize: 11 }} axisLine={false} tickLine={false} interval={1} />
                ...
                <Bar dataKey="Programado" fill="url(#gProg)" radius={[3,3,0,0]} maxBarSize={10} />
                <Bar dataKey="Executado"  fill="url(#gExec)" radius={[3,3,0,0]} maxBarSize={10} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-5 border border-white/5 shadow-xl">
            <h3 className="text-white font-bold text-sm mb-4 uppercase tracking-wide">Top Parceiras — Prog vs Exec</h3>
            <ResponsiveContainer width="100%" height={300}>
              <ComposedChart
                layout="vertical"
              ...
              >
                ...
                <Tooltip content={<ChartTooltip />} />
                <Legend iconType="square" iconSize={8} formatter={v => <span className="text-zinc-400 text-xs">{v}</span>} wrapperStyle={{ paddingTop: 10 }} />
                <Bar dataKey="Programado" fill="url(#gProg2)" radius={[0,3,3,0]} maxBarSize={9} />
                <Bar dataKey="Executado"  fill="url(#gExec2)" radius={[0,3,3,0]} maxBarSize={9} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between">
        <span className="text-white font-bold text-sm uppercase tracking-wide">Detalhamento</span>
        <div className="flex items-center gap-2">
          <button
      ...
      <div className={`grid gap-4 ${openTableData && openTableGroup ? "grid-cols-2" : "grid-cols-1"}`}>
          ...
          <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl border border-white/5 shadow-xl overflow-hidden flex flex-col">
            <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <svg className={`w-3 h-3 transition-transform duration-200 ${showAllColumns ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h7" />
                  </svg>
                  {showAllColumns ? "Menos colunas" : "Mais colunas"}
                </button>
                <span className="text-xs text-zinc-500 bg-white/5 px-2 py-0.5 rounded-full">{data.length} dias</span>
              </div>
            </div>
            {/* Filtros da tabela por data */}
            <div className="px-4 py-2.5 border-b border-white/5 flex items-center gap-3 bg-[#0f1e2e]/40">
              <span className="text-zinc-600 text-xs uppercase tracking-wider shrink-0">Filtrar:</span>
              <button
              ...
              <div className="relative">
                <select
                  value={filterPerf}
                  onChange={e => setFilterPerf(e.target.value as "all" | "above" | "below")}
                ...
                <svg className="pointer-events-none absolute right-1.5 top-1/2 -translate-y-1/2 w-3 h-3 text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </div>
              {(filterWeekday || filterPerf !== "all") && (
                <button onClick={() => { setFilterWeekday(false); setFilterPerf("all"); }} className="text-zinc-500 hover:text-zinc-300 text-xs transition-colors">
                  Limpar
                </button>
              )}
            </div>
              ...
              <table className="w-full text-xs">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-[#0f1e2e]">
                    ...
                    {showAllColumns && (
                      <>
                        <th colSpan={2} className="py-2 px-3 text-center text-[#3b82f6] font-bold text-xs uppercase tracking-wider border-x border-[#3b82f6]/20">
                          Meta 100%
                        </th>
                        <th colSpan={2} className="py-2 px-3 text-center text-[#a78bfa] font-bold text-xs uppercase tracking-wider border-x border-[#a78bfa]/20">
                          Meta 108%
                        </th>
                      </>
                    )}
                  ...
                  <tr className="border-b border-white/5 bg-[#0f1e2e]/80">
                    {["Data","Dia","Obras","Equipes"].map(h => (
                      <th key={h} className="text-left py-2.5 px-3 text-zinc-500 font-semibold uppercase tracking-wider text-xs">{h}</th>
                    ))}
                    {showAllColumns && (
                      <>
                        <th className="text-left py-2.5 px-3 text-[#3b82f6]/70 font-semibold text-xs">Valor</th>
                        <th className="text-left py-2.5 px-3 text-[#3b82f6]/70 font-semibold text-xs">% Dia</th>
                        <th className="text-left py-2.5 px-3 text-[#a78bfa]/70 font-semibold text-xs">Valor</th>
                        <th className="text-left py-2.5 px-3 text-[#a78bfa]/70 font-semibold text-xs">% Dia</th>
                      </>
                    )}
                    <th className="text-left py-2.5 px-3 text-zinc-500 font-semibold text-xs">Prog.</th>
                    <th className="text-left py-2.5 px-3 text-zinc-500 font-semibold text-xs">Exec.</th>
                  </tr>
                </thead>
                <tbody>
                  {data
                    ...
                    .filter(({ row }) => {
                      if (filterWeekday && isWeekend(row.dataProg)) return false;
                      if (filterPerf === "above" && metaDiaria > 0) return (row.totalMoProg / metaDiaria) * 100 >= 100;
                      if (filterPerf === "below" && metaDiaria > 0) return (row.totalMoProg / metaDiaria) * 100 < 100;
                      return true;
                    })
                    .map(({ row, i }) => {
                    const meta100 = metaDiaria;
                    const meta108 = metaDiaria * 1.08;
                    const pct100  = meta100 > 0 ? (row.totalMoProg / meta100) * 100 : 0;
                    const pct108  = meta108 > 0 ? (row.totalMoProg / meta108) * 100 : 0;
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-white/[0.03] cursor-pointer transition-all duration-150 ${
                          ...
                          : weekend ? "bg-[#354a60]/10 hover:bg-[#354a60]/20"
                          : "hover:bg-white/[0.04]"
                        }`}
                      >
                        <td className={`py-2 px-3 font-bold text-xs ${weekend ? "text-zinc-500" : "text-[#53FF75]"}`}>{row.dataProg}</td>
                        <td className="py-2 px-3 text-zinc-500 text-xs">{getDayOfWeek(row.dataProg)}</td>
                        <td className="py-2 px-3 text-zinc-300 font-semibold text-xs">{row.totalQtde}</td>
                        <td className="py-2 px-3 text-zinc-300 font-semibold text-xs">{row.teamsTotal ?? 0}</td>
                        {showAllColumns && (
                          <>
                            <td className="py-2 px-3 text-zinc-400 text-xs">{BRL(meta100)}</td>
                            <td className="py-2 px-3">
                              <div className="flex items-center gap-1.5">
                                <span className="font-bold px-1.5 py-0.5 rounded-full text-xs" style={{ background: c100.bg, color: c100.text }}>{pct100.toFixed(0)}%</span>
                                <div className="w-8 h-1 bg-white/5 rounded-full overflow-hidden">
                                  <div className="h-full rounded-full" style={{ width: `${Math.min(pct100,100)}%`, background: c100.bar }} />
                                </div>
                              </div>
                            </td>
                            <td className="py-2 px-3 text-zinc-400 text-xs">{BRL(meta108)}</td>
                            <td className="py-2 px-3">
                              <div className="flex items-center gap-1.5">
                                <span className="font-bold px-1.5 py-0.5 rounded-full text-xs" style={{ background: c108.bg, color: c108.text }}>{pct108.toFixed(0)}%</span>
                                <div className="w-8 h-1 bg-white/5 rounded-full overflow-hidden">
                                  <div className="h-full rounded-full" style={{ width: `${Math.min(pct108,100)}%`, background: c108.bar }} />
                                </div>
                              </div>
                            </td>
                          </>
                        )}
                        <td className="py-2 px-3 text-zinc-400 text-xs">{BRL(row.totalMoProg)}</td>
                        <td className="py-2 px-3 text-zinc-400 text-xs">{BRL(row.totalMoExec)}</td>
                      </tr>
                    );
                  })}
                  {data.length > 0 && (() => {
                    ...
                    return (
                      <tr className="border-t-2 border-[#53FF75]/20 bg-[#053715]/20">
                        <td colSpan={2} className="py-3 px-3 font-black text-[#53FF75] uppercase text-xs tracking-wider">TOTAL</td>
                        <td className="py-3 px-3 font-black text-white text-xs">{totalObras}</td>
                        <td className="py-3 px-3 font-black text-white text-xs">{totalEquipes}</td>
                        {showAllColumns && (
                          <>
                            <td className="py-3 px-3 font-black text-white text-xs">{BRL(t100)}</td>
                            <td className="py-3 px-3"><span className="font-black px-2 py-0.5 rounded-full text-xs" style={{ background: c100.bg, color: c100.text }}>{p100.toFixed(0)}%</span></td>
                            <td className="py-3 px-3 font-black text-white text-xs">{BRL(t108)}</td>
                            <td className="py-3 px-3"><span className="font-black px-2 py-0.5 rounded-full text-xs" style={{ background: c108.bg, color: c108.text }}>{p108.toFixed(0)}%</span></td>
                          </>
                        )}
                        <td className="py-3 px-3 font-black text-white text-xs">{BRL(totalProg)}</td>
                        <td className="py-3 px-3 font-black text-white text-xs">{BRL(totalExec)}</td>
                      </tr>
                    );
                  })()}
                </tbody>
        ...
        {openTableGroup && (
          <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl border border-white/5 shadow-xl overflow-hidden flex flex-col">
            <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-500 bg-white/5 px-2 py-0.5 rounded-full">{filteredData2.length} linhas</span>
            </div>
            <div className="overflow-auto max-h-[520px]">
              <table className="w-full text-xs">
                <thead className="sticky top-0 z-10">
                  <tr className="border-b border-white/5 bg-[#0f1e2e]/80">
                    {["Grupo","Parceira","Programado","Executado","Previsto","%"].map(h => (
                      <th key={h} className="text-left py-2.5 px-3 text-zinc-500 font-semibold uppercase tracking-wider text-xs">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[...filteredData2]
                    .sort((a, b) => b.totalMoProg - a.totalMoProg)
                    .map((row, i) => {
                      const pct = row.totalMoProg > 0 ? (row.totalMoExec / row.totalMoProg) * 100 : 0;
                      const { bg, text, bar } = pctColor(pct);
                      return (
                        <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.04] transition-colors">
                          <td className="py-2 px-3 text-zinc-300 font-semibold text-xs">{row.grupo}</td>
                          <td className="py-2 px-3 font-bold text-[#53FF75] text-xs">{row.turma}</td>
                          <td className="py-2 px-3 text-zinc-400 text-xs">{BRL(row.totalMoProg)}</td>
                          <td className="py-2 px-3 text-zinc-400 text-xs">{BRL(row.totalMoExec)}</td>
                          <td className="py-2 px-3 text-zinc-400 text-xs">{BRL(row.totalMoPrev)}</td>
                          <td className="py-2 px-3">
                            <div className="flex items-center gap-2">
                              <span className="font-bold px-1.5 py-0.5 rounded-full text-xs whitespace-nowrap" style={{ background: bg, color: text }}>
                                {pct.toFixed(0)}%
                              </span>
