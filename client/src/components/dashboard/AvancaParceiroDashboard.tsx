...
const EXCLUDE_PARCEIRAS = new Set([
  "EDP", "ELETROREDE", "MONTELBRAS", "OCA", "NÃO DEFINIDO", "NAO DEFINIDO",
]);
...
function MultiSelect<T extends FilterOption>({
  ...
  label: string; options: T[]; displayKey: keyof T;
  ...
  const labelDisplay =
    ...
    : selected.length === 1 ? String(options.find((o) => o.id === selected[0])?.[displayKey] ?? "")
    ...
    <div className="bg-[#0f1e2e]/95 border border-white/10 rounded-xl px-4 py-3 text-xs shadow-2xl backdrop-blur-sm">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex justify-between gap-6">
          ...
          <span className="font-bold text-emerald-400">{fmt(row?.executada ?? 0)}</span>
        </div>
        <div className="flex justify-between gap-6">
          <span className="text-amber-400">Executada parcial:</span>
          <span className="font-bold text-amber-400">{fmt(row?.executada_parcial ?? 0)}</span>
        </div>
        <div className="flex justify-between gap-6">
          <span className="text-red-400">Não executada:</span>
          <span className="font-bold text-red-400">{fmt(row?.nao_executada ?? 0)}</span>
        </div>
        <div className="flex justify-between gap-6">
          <span className="text-zinc-500">Não informada:</span>
          <span className="font-bold text-zinc-400">{fmt(row?.nao_informada ?? 0)}</span>
        </div>
...
export default function AvancaParceiroDashboard({
  ...
  const [eliminacao, setEliminacao] = useState<EliminacaoRow[]>(
    (initialEliminacao ?? []).map((r) => ({ ...r, pct: pctExact(r.sem_restricao, r.total) }))
  );
  const [aderencia, setAderencia] = useState<AderenciaRow[]>(initialAderencia ?? []);

  const [motivos, setMotivos] = useState<MotivoRow[] | null>(null);
  const [loadingMotivos, setLoadingMotivos] = useState(false);
  const [showMotivos, setShowMotivos] = useState(false);

  const optRegional = useMemo(() => (filtersData.regional ?? []).map((r) => ({ id: String(r.id), regional: r.regional })), [filtersData]);
  ...
  const applyFilters = useCallback(async () => {
    ...
    try {
      ...
      const [r1, r2] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/restricao/eliminacao-restricao?${params}`, opts),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/restricao/aderencia-parceira?${params}`, opts),
      ]);
      const [j1, j2] = await Promise.all([r1.json(), r2.json()]);

      if (r1.ok) setEliminacao((j1.data ?? []).map((r: any) => ({ ...r, pct: pctExact(r.sem_restricao, r.total) })));
  ...
  const fetchMotivos = useCallback(async () => {
    ...
    if (showMotivos) { setShowMotivos(false); return; }
  ...
  }, [token, dataInicial, dataFinal, selRegional, selParceira, loadingMotivos, showMotivos]);

  // ── KPIs ────────────────────────────────────────────────────────────────

  const kpiEliminacao = useMemo(() => {
    const total = eliminacao.reduce((s, r) => s + r.total, 0);
    const sem = eliminacao.reduce((s, r) => s + r.sem_restricao, 0);
    return { pct: pctExact(sem, total), total, sem };
  }, [eliminacao]);

  const kpiAderencia = useMemo(() => {
    const total = aderencia.reduce((s, r) => s + r.total, 0);
    const exec = aderencia.reduce((s, r) => s + r.executada, 0);
    const parcial = aderencia.reduce((s, r) => s + r.executada_parcial, 0);
    const naoExec = aderencia.reduce((s, r) => s + r.nao_executada, 0);
    const naoInf = aderencia.reduce((s, r) => s + r.nao_informada, 0);
    return { pct: pctExact(exec, total), total, exec, parcial, naoExec, naoInf };
  }, [aderencia]);
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-[#0f2744] to-[#1e3a5f] rounded-2xl p-5 border border-white/5 shadow-lg flex flex-col gap-1">
          <span className="text-white/60 text-xs uppercase tracking-widest">Obras Programadas</span>
          <span className="text-3xl font-black text-white">{fmt(kpiAderencia.total)}</span>
          <span className="text-zinc-500 text-xs">total no período</span>
        </div>
        <div className={`rounded-2xl p-5 border border-white/5 shadow-lg flex flex-col gap-1 ${kpiAderencia.pct >= 70 ? "bg-gradient-to-br from-[#052e16] to-[#14532d]" : "bg-gradient-to-br from-[#431407] to-[#7c2d12]"}`}>
          <span className="text-white/60 text-xs uppercase tracking-widest">Aderência Parceira</span>
          <span className="text-3xl font-black text-white">{kpiAderencia.pct}%</span>
          <span className="text-zinc-500 text-xs">{fmt(kpiAderencia.exec)} executadas · {fmt(kpiAderencia.parcial)} parciais / {fmt(kpiAderencia.total)} programadas</span>
        </div>
        <div className={`rounded-2xl p-5 border border-white/5 shadow-lg flex flex-col gap-1 ${kpiEliminacao.pct >= 70 ? "bg-gradient-to-br from-[#052e16] to-[#14532d]" : "bg-gradient-to-br from-[#431407] to-[#7c2d12]"}`}>
          <span className="text-white/60 text-xs uppercase tracking-widest">Eliminação de Restrições</span>
          <span className="text-3xl font-black text-white">{kpiEliminacao.pct}%</span>
          <span className="text-zinc-500 text-xs">{fmt(kpiEliminacao.sem)} sem restrição / {fmt(kpiEliminacao.total)} total</span>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-6 border border-white/5 shadow-xl">
        <div className="mb-4">
          <h3 className="text-white font-bold text-sm tracking-wide uppercase">Gráfico 1 — Eliminação de Restrições</h3>
          <p className="text-zinc-500 text-xs mt-0.5">Obras sem restrição de programação ativa vs total, por mês</p>
        </div>
        {eliminacao.length === 0 ? (
          <div className="flex items-center justify-center h-48 text-zinc-500 text-sm">Nenhum dado encontrado para o período selecionado.</div>
        ) : (
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={eliminacao} margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" />
              <XAxis dataKey="mes" tick={{ fill: "#94a3b8", fontSize: 11 }} />
              <YAxis yAxisId="left" tick={{ fill: "#94a3b8", fontSize: 11 }} />
              <YAxis yAxisId="right" orientation="right" domain={[0, 100]} tickFormatter={(v) => `${v}%`} tick={{ fill: "#94a3b8", fontSize: 11 }} />
              ...
              <Line yAxisId="right" type="monotone" dataKey="pct" name="% Eliminação" stroke="#3b82f6" strokeWidth={2} dot={{ fill: "#3b82f6", r: 4 }} activeDot={{ r: 6 }} />
            </ComposedChart>
          </ResponsiveContainer>
        )}
      </div>
      ...
      <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-6 border border-white/5 shadow-xl">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h3 className="text-white font-bold text-sm tracking-wide uppercase">Gráfico 2 — Aderência Parceira</h3>
            <p className="text-zinc-500 text-xs mt-0.5">
              Status de execução das obras programadas (prog vs exec), por semana
            </p>
          </div>
          <button
          ...
          >
            {loadingMotivos ? "Carregando…" : showMotivos ? "Ocultar motivos" : "Ver motivos de reprogramação"}
          </button>
        </div>
        ...
        {aderencia.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={300}>
            <ComposedChart data={aderencia} margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" />
              <XAxis dataKey="semana" tick={{ fill: "#94a3b8", fontSize: 10 }} interval="preserveStartEnd" />
              <YAxis yAxisId="left" tick={{ fill: "#94a3b8", fontSize: 11 }} />
              <YAxis yAxisId="right" orientation="right" domain={[0, 100]} tickFormatter={(v) => `${v}%`} tick={{ fill: "#94a3b8", fontSize: 11 }} />
              <Tooltip content={<TooltipAderencia />} />
              <Legend wrapperStyle={{ fontSize: 12, color: "#94a3b8", paddingTop: 12 }} />
              <Bar yAxisId="left" dataKey="executada" name="Executada" stackId="a" fill="#10b981" />
              <Bar yAxisId="left" dataKey="executada_parcial" name="Executada parcial" stackId="a" fill="#f59e0b" />
              <Bar yAxisId="left" dataKey="nao_executada" name="Não executada" stackId="a" fill="#ef4444" />
              <Bar yAxisId="left" dataKey="nao_informada" name="Não informada" stackId="a" fill="#6b7280" radius={[4, 4, 0, 0]} />
              <Line yAxisId="right" type="monotone" dataKey="pct" name="Aderência %" stroke="#3b82f6" strokeWidth={2} dot={{ fill: "#3b82f6", r: 3 }} activeDot={{ r: 5 }} />
            </ComposedChart>
          </ResponsiveContainer>
        )}
        ...
        {showMotivos && motivos && (
          <div className="mt-6 bg-[#0f1e2e]/60 rounded-xl border border-white/10 p-4">
            <h4 className="text-white font-semibold text-xs uppercase tracking-wider mb-3">
              ...
              <span className="ml-2 text-zinc-500 font-normal normal-case">({motivos.length} registro{motivos.length !== 1 ? "s" : ""})</span>
            </h4>
            {motivosChart.length === 0 ? (
              <p className="text-zinc-500 text-xs">Nenhum motivo de reprogramação para o período selecionado.</p>
            ) : (
              <ResponsiveContainer width="100%" height={Math.min(400, motivosChart.length * 44 + 60)}>
                <ComposedChart
                  layout="vertical"
                  data={motivosChart}
