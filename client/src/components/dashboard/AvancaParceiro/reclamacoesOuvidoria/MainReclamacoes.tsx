...
export default function MainReclamacoes() {
  ...
  const handleFile = (file: File) => {
    startTransition(async () => {
      try {
        ...
        triggerNotification(
          `${parsed.rows.length} reclamações importadas da aba "${parsed.sheetName}".`,
          "success",
        );
      } catch (err) {
        setError(
  ...
  const regionalOptions = useMemo(
    () => distinctValues(rows, "regional"),
    [rows],
  );
  const empreiteiraOptions = useMemo(
    () => distinctValues(rows, "empreiteira"),
    [rows],
  );
  const tipoOptions = useMemo(
    () => distinctValues(rows, "tipoReclamacao"),
    [rows],
  );
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const matchesEmpreiteira =
        !filterEmpreiteira || row.empreiteira === filterEmpreiteira;
      const matchesTipo = !filterTipo || row.tipoReclamacao === filterTipo;

      const abertura = row.dataAbertura ? dayjs(row.dataAbertura) : null;
      const matchesStart =
        !startDate || (abertura && !abertura.isBefore(startDate, "day"));
      const matchesEnd =
        !endDate || (abertura && !abertura.isAfter(endDate, "day"));
  ...
  const metrics = useMemo(
    () => buildReclamacoesMetrics(filteredRows),
    [filteredRows],
  );
