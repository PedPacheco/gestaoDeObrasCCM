...
import { DataDashboardInterface } from "../DashboardClient";
import { KpiSectionOverviem } from "./KpiSection";
import { OverviewDashboardFilters } from "./overviewDashboardFilters";
...
export function OverviewDashboard({
  ...
  const { clearFilters, filters, saveFilters } = useSaveFilters({
    pageKey: "OverviewDashboardFilters",
    data: filtersData,
  });

  const [data, setData] = useState<DataDashboardInterface>(initialData ?? {});
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      />
      {/* ── KPI Cards ─────────────────────────────────────────────── */}
      <KpiSectionOverviem kpis={data.kpis} />
              ...
              <defs>
                {data.topPartners.map((_p, i) => (
                  ...
                  >
                    <stop
                      offset="0%"
                      stopColor={FALLBACK_COLORS[i % FALLBACK_COLORS.length]}
                      stopOpacity={0.7}
                    />
                    <stop
                      offset="100%"
