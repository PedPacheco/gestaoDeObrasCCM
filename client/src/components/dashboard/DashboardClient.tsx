...
const FALLBACK_COLORS = ["#53FF75","#3b82f6","#f59e0b","#ef4444","#8b5cf6","#06b6d4","#f97316","#ec4899","#14b8a6"];
...
interface ByRegional  { regional: string; total: number; concluded: number }
interface Trend       { month: string; entered: number; concluded: number }
interface TopPartner  { partner: string; total: number }
interface RecentWork  { ovnota: string; status: string; partner: string; municipio: string; executado: number | null; entrada: string }
...
function KpiCard({ label, value, sub, gradient, accent }: {
  ...
  return (
    <div className={`relative rounded-2xl p-5 flex flex-col gap-2 overflow-hidden shadow-lg ${gradient}`}>
      <div className="absolute top-0 left-0 w-1 h-full rounded-l-2xl" style={{ background: accent }} />
      <span className="text-white/60 text-xs uppercase tracking-widest font-medium pl-2">{label}</span>
      <span className="text-4xl font-black text-white pl-2 leading-none">{value}</span>
    ...
    <div className="bg-[#0f1e2e]/95 border border-white/10 rounded-xl px-4 py-3 text-xs text-zinc-100 shadow-2xl backdrop-blur-sm">
      ...
      {payload.map((p: any, i: number) => (
        <div key={i} className="flex items-center gap-2 py-0.5">
          ...
          <span className="font-bold text-white">{p.value?.toLocaleString("pt-BR")}</span>
        </div>
      ))}
    </div>
  );
...
function ChartCard({ title, children, className = "" }: { title: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={`bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-6 border border-white/5 shadow-xl ${className}`}>
      <h3 className="text-white font-bold text-sm mb-5 tracking-wide uppercase">{title}</h3>
      {children}
    </div>
  );
}
...
export default function DashboardClient({
  ...
  token, initialMaodeObra, initialMaodeObra2, initialMetaDiaria,
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#111c27]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <defs>
                {byStatus.map((entry, i) => (
                  <linearGradient key={i} id={`statusGrad${i}`} x1="0" y1="0" x2="1" y2="1">
                    <stop offset="0%" stopColor={getStatusColor(entry.status, i)} stopOpacity={1} />
                    <stop offset="100%" stopColor={getStatusColor(entry.status, i)} stopOpacity={0.75} />
                  </linearGradient>
                ))}
              </defs>
              <Pie
        ...
        <ChartCard title="Obras por Regional">
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={byRegional} layout="vertical" margin={{ left: 0, right: 20, top: 4, bottom: 4 }} barGap={4}>
              <defs>
              ...
              <Legend
                ...
                wrapperStyle={{ paddingTop: "12px" }}
              />
              <Bar dataKey="total" name="Total" fill="url(#gradTotal)" radius={[0, 6, 6, 0]} maxBarSize={14} />
              <Bar dataKey="concluded" name="Concluídas" fill="url(#gradConcluded)" radius={[0, 6, 6, 0]} maxBarSize={14} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
      </div>
      ...
      <ChartCard title="Tendência — Entradas vs Conclusões (últimos 6 meses)">
        <ResponsiveContainer width="100%" height={230}>
          <AreaChart data={trendFormatted} margin={{ left: 0, right: 20, top: 8, bottom: 0 }}>
            <defs>
              <linearGradient id="areaEntered" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#3b82f6" stopOpacity={0.35} />
                <stop offset="100%" stopColor="#3b82f6" stopOpacity={0} />
              </linearGradient>
              <linearGradient id="areaConcluded" x1="0" y1="0" x2="0" y2="1">
            ...
            <Area
              ...
              activeDot={{ r: 6, fill: "#53FF75", stroke: "#fff", strokeWidth: 2 }}
            />
          </AreaChart>
        </ResponsiveContainer>
      </ChartCard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            >
              <defs>
                {topPartners.map((_p, i) => (
                  <linearGradient key={i} id={`partnerGrad${i}`} x1="0" y1="0" x2="1" y2="0">
                    <stop offset="0%" stopColor={FALLBACK_COLORS[i % FALLBACK_COLORS.length]} stopOpacity={0.7} />
                    <stop offset="100%" stopColor={FALLBACK_COLORS[i % FALLBACK_COLORS.length]} stopOpacity={1} />
                  </linearGradient>
                ))}
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" horizontal={false} />
              ...
              />
              <Tooltip content={<ChartTooltip />} />
              <Bar dataKey="total" name="Obras" radius={[0, 6, 6, 0]} maxBarSize={16}>
                {topPartners.map((_, i) => (
        ...
        <div className="bg-gradient-to-br from-[#1e2f42] to-[#192535] rounded-2xl p-6 border border-white/5 shadow-xl flex flex-col">
          <h3 className="text-white font-bold text-sm mb-5 tracking-wide uppercase">Últimas Obras Registradas</h3>
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-white/5">
                  <th className="text-left pb-3 pr-3 text-zinc-500 font-semibold uppercase tracking-wider text-[10px]">Nota</th>
                  <th className="text-left pb-3 pr-3 text-zinc-500 font-semibold uppercase tracking-wider text-[10px]">Status</th>
                  <th className="text-left pb-3 pr-3 text-zinc-500 font-semibold uppercase tracking-wider text-[10px]">Parceira</th>
                  <th className="text-left pb-3 pr-3 text-zinc-500 font-semibold uppercase tracking-wider text-[10px]">Município</th>
                  <th className="text-right pb-3 text-zinc-500 font-semibold uppercase tracking-wider text-[10px]">Exec.</th>
                </tr>
              </thead>
              <tbody>
                {recentWorks.map((w, i) => (
                  <tr
                    ...
                    onClick={() => window.open(`/detalhes/${w.ovnota}`, "_blank")}
                  >
                    <td className="py-2.5 pr-3 font-bold text-[#53FF75] group-hover:text-white transition-colors">{w.ovnota}</td>
                    <td className="py-2.5 pr-3 text-zinc-400 truncate max-w-[110px]">{w.status}</td>
                    <td className="py-2.5 pr-3 text-zinc-400 truncate max-w-[90px]">{w.partner}</td>
                    <td className="py-2.5 pr-3 text-zinc-400 truncate max-w-[90px]">{w.municipio}</td>
                    <td className="py-2.5 text-right">
                      {w.executado != null ? (
                        <span
                          className="font-bold px-2 py-0.5 rounded-full text-[10px]"
                          style={{
                            background: w.executado >= 80 ? "#053715" : w.executado >= 40 ? "#431407" : "#1e1b4b",
                            color: w.executado >= 80 ? "#53FF75" : w.executado >= 40 ? "#f97316" : "#818cf8",
                          }}
                        >
                          {w.executado}%
                        </span>
