...
export const INITIAL_EXECUTION_REPORT: OldExecutionReportData = {
  id: 0,
  idUser: 0,
  supervisor: "",
...
interface UseOldScheduleFormProps {
  data?: FormData;
  executionData: OldExecutionReportData;
  options: ScheduleFormDialogProps["options"];
}

export const useOldScheduleForm = ({
  data,
  executionData,
  options,
}: UseOldScheduleFormProps) => {
  const [formData, setFormData] = useState<FormData>(INITIAL_FORM_DATA);
  const [executionReportData, setExecutionReportData] =
    useState<OldExecutionReportData>(INITIAL_EXECUTION_REPORT);
  ...
  useEffect(() => {
    if (data) {
      const mapped = OldMapScheduleToForm(data, options);
      setFormData(mapped);
    }
        ...
        setFormData((prev) => {
          if (field.startsWith("executionReport.")) {
            const subField = field.split(
              ".",
            )[1] as keyof OldExecutionReportData;
