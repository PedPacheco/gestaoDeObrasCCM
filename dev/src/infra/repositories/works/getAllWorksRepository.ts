...
export class GetAllWorksRepository implements IGetAllWorksRepository {
  ...
  async getAllWorks(
    ...
    let query = Prisma.sql`SELECT
        ...
        entrada + prazo AS prazo_fim, qtde_planejada, mo_planejada, mo_final, turma, executado, data_conclusao, status, observ_obra, referencia 
        ${baseQuery}`;
    ...
    const [works, total] = await Promise.all([
      this.prisma.$queryRaw<GetAllWorksItem[]>(query),
