...
export class GetWorksInPortfolioRepository implements IGetWorksInPortfolioRepository {
  ...
  async getWorksInPortfolio(
    ...
    let query = Prisma.sql`SELECT
        obras.id, obras.ovnota, COALESCE(diagrama, COALESCE(ordem_dci, ordem_dcim)) AS ordem_principal, ordem_dca, ordem_dcd, ordem_dcim, COALESCE(diagrama, ordem_dci, ordem_dcim, ordem_dcd, ordem_dca) AS ordemdiagrama, 
    ...
    query = Prisma.sql`${query} GROUP BY obras.id, ovnota, diagrama, ordem_dci, ordem_dcim, ordem_dca, ordem_dcd, status_ov_sap, pep,
        mun, id_status, prazo, abrev_regional, tipo_obra, tipos.id_grupo, qtde_planejada, qtde_pend, 
