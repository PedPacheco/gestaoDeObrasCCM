...
describe('FeasibilityService', () => {
  ...
  describe('exportFeasibility', () => {
    it('should return the data formatted for export', async () => {
      ...
      const result = await service.exportFeasibility(
        '20/05/2026',
        '22/05/2026',
        [2],
      );

      expect(mockRepository.exportFeasibility).toHaveBeenCalledWith(
        '20/05/2026',
        '22/05/2026',
        [2],
      );
    ...
    // it('should use material data when servicos_contratos is null', async () => {
    //   const result = await service.exportFeasibility(
    //     '20/05/2026',
    //     '22/05/2026',
    //     [2],
    ...
    //   const materialRow = result.find((item) => item.tipo === 'M');

    //   expect(materialRow?.material).toBe('MAT-001');
    //   expect(materialRow?.textoBreve).toBe('Cabo multiplexado');
    // });
  });
});
