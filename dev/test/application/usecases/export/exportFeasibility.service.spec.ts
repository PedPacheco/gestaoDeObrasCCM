...
describe('ExportFeasibilityService', () => {
  ...
  describe('exportFeasibilityPendingApproval', () => {
    it('should create workbook, add worksheet, set columns and write response', async () => {
      ...
      await service.exportFeasibilityPendingApproval(data, mockResponse);
    ...
    it('should handle empty works array', async () => {
      await service.exportFeasibilityPendingApproval([], mockResponse);
    ...
    it('should throw error if write fails', async () => {
      ...
      await expect(
        service.exportFeasibilityPendingApproval(
          [{ ovnota: '1' }],
          mockResponse,
        ),
  ...
  describe('exportFeasibilityPending', () => {
    it('should create workbook, add worksheet, set columns and write response', async () => {
      ...
      await service.exportFeasibilityPending(data, mockResponse);

      expect(ExcelJS.Workbook).toHaveBeenCalled();

      expect(addWorksheetMock).toHaveBeenCalledWith(
        'Obras aguardando viabilidade',
      );
    ...
    it('should handle empty works array', async () => {
      await service.exportFeasibilityPending([], mockResponse);
    ...
    it('should throw error if write fails', async () => {
      ...
      await expect(
        service.exportFeasibilityPending([{ ovnota: '1' }], mockResponse),
      ).rejects.toThrow('write error');
    });
